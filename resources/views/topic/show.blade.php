            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="{{route('submit',['id'=>$topic->id])}}" method="POST">
                           {{csrf_field()}}
                ...
                <div class="tab-content">
                    <div class="tab-pane active" id="tab_1">
                        @foreach($posts as $post)
                            ...
                            <p class=""><a href="{{route('show',['id'=>$post->id])}}">{{$topic->name}}</a> {{$post->created_at->diffForHumans()}}</p>
                            <p class=""><a href="{{route('show',['id'=>$post->id])}}" >{{$post->title}}</a></p>
